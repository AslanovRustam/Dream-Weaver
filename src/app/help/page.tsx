...
// ─────────────────────────────────────────────────────────────────────────────
// Content — realistic FAQ tied to what the product actually does (banner/landing/
// playable/video/email generators, ТЗ upload, mailing cabinet, ad accounts &
// stats, credits, account). Grouped by topic; each item has a stable id so search can
...
const FAQ_GROUPS: FaqGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: "gen-brief",
        q: "Как загрузить ТЗ и заполнить поля автоматически?",
        a: "В любом генераторе (баннер, лендинг, playable, видео, письма) вверху настроек есть блок «Загрузить ТЗ». Вставьте текст задания или прикрепите файл .docx / .pdf / .txt и нажмите «Разобрать ТЗ» — ИИ прочитает бриф и предложит два действия: «Заполнить поля» (подставит извлечённые значения в форму) или «Сгенерировать продукт» (заполнит поля и сразу запустит генерацию). Формат .doc не поддерживается — пересохраните как .docx.",
      },
      {
        id: "gen-resizes",
  ...
  {
    id: "email",
    title: "Письма и рассылки",
    icon: Mail,
    items: [
      {
        id: "email-create",
        q: "Как собрать письмо для рассылки?",
        a: "Откройте раздел «Генератор писем». Заполните тему, прехедер, бренд, заголовок, текст и кнопку — справа сразу обновляется живой предпросмотр настоящего письма. Выберите стиль (промо, дайджест, анонс, минимал) и акцентный цвет, затем нажмите «Сохранить письмо». Сохранённые письма доступны в кабинете рассылок.",
      },
      {
        id: "email-send",
        q: "Как отправить рассылку?",
        a: "В разделе «Кабинет рассылок» в блоке «Создать рассылку» выберите сохранённое письмо и аудиторию, проверьте число получателей и нажмите «Отправить рассылку». Кампания появится в таблице ниже со статусом «Отправлена» и статистикой доставки.",
      },
      {
        id: "email-audiences",
        q: "Какие аудитории доступны?",
        a: "Аудитории — это сегменты подписчиков: «Все подписчики», «VIP-игроки», «Депозиторы», «Неактивные 30 дней», «Новые (7 дней)». У каждой показан размер. Аудитория выбирается при создании рассылки и определяет число получателей.",
      },
      {
        id: "email-stats",
        q: "Где смотреть открытия и клики?",
        a: "В кабинете рассылок вверху — KPI за выбранный период (отправлено, доставлено, открытия, клики, Open Rate, CTR) и график открытий и кликов по дням. По каждой кампании в таблице видны получатели, открытия, Open Rate, клики, CTR и дата отправки.",
      },
    ],
  },
  {
    id: "ads",
    title: "Рекламные кабинеты и статистика",
    icon: Megaphone,
    items: [
      {
        id: "ads-connect",
        q: "Как подключить рекламный кабинет?",
        a: "Откройте раздел «Рекламные кабинеты» и нажмите «Подключить» на нужной площадке — Meta (Facebook/Instagram), Google Ads или TikTok Ads. Подключённые кабинеты появятся в списке ниже со статусом; там же их можно отключить.",
      },
      {
        id: "ads-stats",
        q: "Что показывает раздел «Статистика»?",
        a: "Сводную аналитику по подключённым кабинетам: KPI (расход, показы, клики, CTR, конверсии, CPC, CPA) с изменением к прошлому периоду, график динамики по выбранной метрике и диапазону дат, а также таблицу кампаний с сортировкой по любому показателю.",
      },
      {
        id: "ads-to-banner",
        q: "Можно ли создать баннер под кампанию?",
        a: "Да — в таблице кампаний в разделе «Статистика» у каждой строки есть кнопка перехода в генератор баннеров, чтобы быстро собрать креатив под эту кампанию.",
      },
      {
        id: "ads-demo",
        q: "Данные кабинетов реальные?",
        a: "Сейчас подключение и статистика работают в демо-режиме — данные симулируются для демонстрации интерфейса. Реальная авторизация площадок и выгрузка статистики подключаются через их OAuth и API-ключи.",
      },
    ],
  },
  {
